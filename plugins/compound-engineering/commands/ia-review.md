...
Run the Task ia-code-simplicity-reviewer() to see if we can simplify the code. Note: `ia-code-simplicity-reviewer` always runs here -- exclude it from `review_agents` in `compound-engineering.local.md` to avoid running it twice.
