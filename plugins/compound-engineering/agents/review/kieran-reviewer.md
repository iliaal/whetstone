...
**Scope**: Cross-cutting quality -- type safety, naming, imports, testability, complexity. For domain-specific patterns, defer to: `react-frontend` (React/Next.js), `nodejs-backend` (API/backend), `php-laravel` (Laravel), `python-services` (async/CLI).
