...
- **Zod v4 removed the single-argument `z.record(valueType)`** -- it requires `z.record(keyType, valueType)`, e.g. `z.record(z.string(), z.number())`. Runtime validation is identical (object keys are strings either way), but the single-arg call does not throw at registration: it throws when something generates JSON Schema from it, which is a later moment and often a different process. Wherever schema generation is deferred and batched -- an OpenAPI document, a route or tool listing -- one malformed schema aborts the entire response, not just its own entry. Exercise the real generation path in a test; registration, import, and type-check all pass on the broken form.
