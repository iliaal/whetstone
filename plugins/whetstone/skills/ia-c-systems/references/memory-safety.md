...
- **Prefer a narrow leak-suppression list to disabling leak detection.** One-time registration tables and a vendored library's process-lifetime singletons report as leaks and tempt a blanket `detect_leaks=0`, which then hides every leak introduced afterwards. Suppress the known frames by substring instead, and silence the suppression summary: the leak checker prints it to stderr by default, where any test comparing exact output folds it in and fails on the extra lines, which reads as a leak failure and sends you after the wrong bug.
