...
**Fix:** assert the exact optimized value, or seed two input sizes and assert invariance across them; that is the only shape that separates O(1) from O(n). A count invariance proves constant statements, never constant work, so assert a resolved value alongside it.
