...
### An empty array and an absent key collapse at every falsy test

`empty($data['key']) ? null : ...` as an absence test cannot distinguish `{"key": []}` from a key that was never sent, and neither can `?? null`, `isset()`, or a plain truthiness check -- only `array_key_exists()` separates them. So a Remove / Clear-all affordance that sends the emptied collection is a silent no-op: 200, nothing written, and a refetch restores what the user deleted. Removing *some* items works, because a non-empty array is not `empty`, so the defect is exactly the remove-all case -- say so, or the report reads as "the whole feature is broken" and cannot be reproduced.
