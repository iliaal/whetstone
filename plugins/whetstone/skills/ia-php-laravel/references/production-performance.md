...
- **Laravel-specific**: `php artisan config:cache && php artisan route:cache && php artisan view:cache && php artisan event:cache`; run on every deploy. `composer install --optimize-autoloader --no-dev` for production. `config:cache` is what makes the no-`env()`-outside-`config/` rule mandatory: after it runs, any `env()` call elsewhere returns `null`.
