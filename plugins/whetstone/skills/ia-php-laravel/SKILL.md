...
- **Adding a constructor parameter to a `ShouldQueue` job breaks every payload already queued, and a promoted default does not save it.** `unserialize()` never runs the constructor and restores only declaration-level defaults, which a promoted property has none of -- `handle()` throws `must not be accessed before initialization`, and `failed_jobs` keeps the stale payload, so every retry re-throws. Fix: a non-promoted, non-`readonly` property with a declaration-level default, assigned in the constructor body. Full mechanism in [pitfalls-deep.md](./references/pitfalls-deep.md).
...
2. Passes solo but fails in the suite → suspect shared state: container singletons, statics, `Carbon::setTestNow()` residue, DB state leaking between tests (the classic paratest failure). A `private static` memo is the sharp case: it is process-scoped, so no database rollback reaches it, and the symptom is an absence assertion that passes alone. Reset it for that one test through reflection rather than deleting the memo -- the per-request cache is correct production behaviour ([testing.md](./references/testing.md)).
...
**A database-level FK cascade fires no Eloquent events.** `->cascadeOnDelete()` is a constraint: the database removes the children, Eloquent never loads them, so no observer, `Auditable` trait, search sync, or storage cleanup runs for them -- while the parent's own delete IS audited, so the log looks populated. When a change adds a delete path on a parent, ask whether children die by cascade or row by row and what hangs off their `deleted` event; delete per row inside the transaction wherever an event-layer invariant must hold. A sibling path already deleting row by row with a comment explaining why is the tell.
...
**`validated()` rebuilds a nested array key from its ruled sub-keys only and drops the rest.** A FormRequest ruling `mapping.first_name` and not `mapping.middle_name` returns `mapping` without `middle_name` -- no error, and unit tests that hand-build the array never cross the FormRequest. Fix: rule every sub-key the consumer reads; when a diff adds a field to a nested payload, grep `rules()` for the dotted key and cover it with a feature test posting the real body. Full mechanism in [pitfalls-deep.md](./references/pitfalls-deep.md).
...
**A `composer.lock` conflict confined to the `content-hash` line is not a lock conflict.** The package arrays merged cleanly; both hashes are stale because each was computed against its own `composer.json`. Never hand-pick a side's hash -- recompute it. Full recipe in [pitfalls-deep.md](./references/pitfalls-deep.md).
