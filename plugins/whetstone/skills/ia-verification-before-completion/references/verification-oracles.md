...
**A command that cannot return a positive has not returned a negative.** A pathspec the tool globs differently than it reads on the page, a filter that discards `command not found`, a wrapper exiting 0 on an empty stream: each yields a clean zero that supports whatever is being tested. Run a positive control through the *same* invocation shape (same tool, ref, filter, shell) and read its output before reading the zero. Never filter stderr on the run that establishes the harness works.
...
**A checker that failed to run is `ERROR`, not `SKIPPED` and not `CLEAN`.** Log creation failed, output could not be parsed, the invocation errored, the binary was missing: each is a failed run, and every per-check status report (a phase table, a gate ledger, a composite score) must carry it as `ERROR`. Verify a checker's availability before running it rather than inferring availability from its exit code.

**A weighted composite score is only as complete as its executed categories.** For any score built from weighted categories with skip-redistribution, a skipped or capture-failed category must be reported as reduced coverage, never silently redistributed into a full clean score. When zero categories executed, report no score at all rather than a numeric one. A trend or delta claim between two runs is valid only when both runs scored an identical category set; differing coverage makes the scores incomparable, and the delta must be withheld with that reason.
