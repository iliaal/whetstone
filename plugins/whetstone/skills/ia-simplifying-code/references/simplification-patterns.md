...
- **Redundant comments** that restate the code (`// increment counter` above `counter++`): delete them. A comment that carries behavior is not redundant even when it sits beside a short line: keep one that says why the line exists, names a workaround or upstream bug, states an invariant, or pins an ordering constraint. Treat directive comments as code (`// @ts-expect-error`, `// eslint-disable-next-line`, `# noqa`, `# type: ignore[...]`, `# pragma: no cover`, `// phpcs:ignore`, `@phpstan-ignore-next-line`, docblock annotations a framework reads); remove one only after rerunning its tool without it and seeing the run stay clean. Keep license and legal headers; no tool rerun can clear their removal
- **Wrong comments** on lines this pass touches (list others as follow-ups): fix or remove one when one of these holds. *Stale*: describes behavior the code no longer has. *Orphaned*: names a symbol, path, flag, or ticket that no longer exists. *Misleading*: states a claim the code contradicts. Rewrite when the correct claim is proven from the code; otherwise delete. Never change code to make a comment true
