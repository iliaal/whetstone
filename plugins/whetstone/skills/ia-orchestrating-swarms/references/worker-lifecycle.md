...
The controller reads all tasks from the plan upfront and provides full task text directly to subagents. Do not make subagents hunt plan files for their assignment; paste the task content into the prompt. Skill availability varies by harness and agent context, so a skill name alone is not an adequate brief. Inline the specific operative instructions; when a worker needs further skill guidance, provide its accessible location and let it inspect that source. See [handoff-templates.md](./handoff-templates.md) for QA FAIL and Escalation Report formats.
