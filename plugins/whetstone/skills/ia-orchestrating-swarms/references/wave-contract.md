...
2. **Exclusive ownership of every write surface, including the hidden ones.** Disjoint declared file lists are not enough. Lockfiles, generated code, snapshots, package manifests, and formatter output are write surfaces too: exclude each from every worker, or assign it to exactly one. Shape shared registries and lists for mergeability: one item per line with a trailing delimiter on every line, inserted in a stable order (alphabetical unless the list is order-sensitive) rather than appended, since appends collide on the same last line. When a conflict lands in a generated file or lockfile anyway, take either side wholesale and regenerate with the owning tool; never hand-resolve it.
