...
# Usage: bash scripts/publish-clawhub.sh [--dry-run] [--skill <name>] [--force]
#
# Prerequisites:
...
#
# Skips skills whose content hasn't changed since the prior release, by
# consulting distillery/.skill-versions.json (regenerated by release.sh
# before this runs). Use --force to publish all skills regardless.
...
MANIFEST="$REPO_ROOT/distillery/.skill-versions.json"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            printf "Usage: %s [--dry-run] [--skill <name>] [--force]\n" "$(basename "$0")"
            printf "\nOptions:\n"
            printf "  --dry-run       Show what would be published without publishing\n"
            printf "  --skill <name>  Publish a single skill instead of all (bypasses change check)\n"
            printf "  --force         Publish all skills regardless of manifest change-detection\n"
            exit 0
            ;;
...
publish_skill() {
    ...
    # Manifest-based change detection: skip if content unchanged since prior release.
    # The skill's content_changed in the manifest tracks the plugin version at which
    # SKILL.md last changed. If that's older than the version we're publishing, the
    # current clawhub copy already reflects identical content under its prior tag.
    # --force, --skill (single-skill mode), and missing manifest all bypass the check.
    # Limitation: references/ and scripts/ changes don't bump content_changed (the
    # manifest hashes only SKILL.md). Use --force or --skill if a references-only
    # edit needs to land on clawhub.
    if [[ "$FORCE" == false && -z "$SINGLE_SKILL" && -f "$MANIFEST" ]]; then
        local content_changed
        content_changed=$(jq -r --arg s "$skill_name" '.skills[$s].content_changed // ""' "$MANIFEST")
        if [[ -n "$content_changed" && "$content_changed" != "$VERSION" ]]; then
            printf "  %-12s %s → %s (unchanged since v%s)\n" "unchanged" "$skill_name" "$slug" "$content_changed"
            skipped=$((skipped + 1))
            return
        fi
    fi
