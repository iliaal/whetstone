...
def _run_claude_code_cli_exec(
    *,
    ...
    cmd = [
        ...
        "--add-dir",
        work_dir,
        # LOCAL VENDOR PATCH (see VENDORED.md): the upstream command also passed
        # `--tools <tools>`, but in claude CLI 2.1.x that flag breaks tool
        # execution -- the model degrades to emitting <function_calls> markup as
        # plain text and never actually runs tools. `--allowedTools` alone gives
        # the correct allow-list behavior.
        "--allowedTools",
        tools,
    ]
