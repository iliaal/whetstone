...
- `ia-planning` adds a Goal Quality Gate as the first section. Before deciding whether to plan, the gate asks 5 questions: what concrete thing will be true; what evidence proves it; what quantitative or binary threshold defines success; what scope boundaries matter; what should cause the agent to stop and ask. Pure-activity goals ("make progress", "keep investigating", "improve things") get rewritten into a verifiable outcome or rejected. The gate skips only when the request already names a specific artifact AND a clear success signal in the user's own words: "fix typo in README line 47", "rename `oldFn` to `newFn` across the repo", "bump lodash to 4.17.21". Anything vaguer runs the gate.
- `ia-planning` "When to Plan" rewrites the skip decision around a 4-condition contract. The old 3-bullet skip rule ("Skip planning: single-file edits, quick lookups, simple questions") becomes: skip only when the work is atomic AND there are no Key Technical Decisions worth recording AND no scope boundaries worth pinning AND no upstream artifact needs traceability. A "bias toward producing a plan" preamble owns the asymmetry: a thin plan for small work is mild ceremony; a missed plan costs real time. Three stress-test examples show requests that look atomic but hide design (add caching hides TTL, invalidation, cache-key shape; migrate package A to B hides semantic differences; add rate limiting hides algorithm, scope, configurability), paired with three genuine skip cases (fix typo, rename function, bump dep version).
- `ia-md-docs` adds a Monorepo Discovery (Authoring) section between Workflows and the existing Monorepo Context Loading. Discovery is recursive by default; `--root-only` collapses to the repo root, and a positional `path` argument scopes to one subtree. The `update-*` workflow enumerates existing `README.md` / `AGENTS.md` via `git ls-files`; the `init-*` workflow finds package roots by manifest pattern (`package.json`, `Cargo.toml`, `pyproject.toml`, `setup.py`, `go.mod`, `composer.json`). Workspace manifests (`pnpm-workspace.yaml`, `package.json` `workspaces:`, `Cargo.toml` `[workspace]`, `go.work`) are the canonical package set when present and beat file enumeration. Per-file scoping: each `AGENTS.md` links to its sibling, not the root one; each gets a sibling `CLAUDE.md` symlink in the same directory.
- `ia-md-docs` Update CONTRIBUTING workflow gains a merge advisory. When `CONTRIBUTING.md` sits next to `AGENTS.md` (repo root or any package root), the workflow surfaces a one-line recommendation to merge its contents into the sibling `AGENTS.md` Contribution Workflow section. Never auto-merges, never auto-deletes; the user performs both. The requested workflow continues regardless; the CONTRIBUTING file is advisory only.
- `ia-lfg` Step 8 (added then renumbered after the sync's reactive audit) distinguishes actionable CI failures from non-actionable gates before polling. `gh pr view --json isDraft,reviewDecision` plus `gh pr checks --json name,state,bucket,conclusion` decide the path. Classification is by check conclusion, not vendor name: `conclusion: failure` on a required status check is actionable and routes to the fix loop; `neutral`/`skipped` or pending non-required checks get ignored. Terminal markers (`DRAFT_PR_WITH_NO_CHECKS`, `BLOCKED_BY_REVIEW_GATE`, `NO_CHECKS_REGISTERED`) stop the agent from waiting on human-approval gates. Required-status-checks (via `gh api repos/{owner}/{repo}/branches/{branch}/protection`) are the source of truth; common bots (Sentry, Codecov, Cursor, BugBot) are examples, not the rule.
- `scripts/post-thread.py` adds image-attach support so tweet-thread posts can carry uploaded images.

### Fixed

- `distillery/scripts/distiller.py` trigger-phrase validation now skips skills and agents that opt out of model-driven invocation (`disable-model-invocation: true`) or auto-load on a path glob (`paths:` frontmatter, string or list value). The `MISSING_TRIGGER` finding only applies to components the model is expected to auto-invoke from the description; slash-only and path-triggered skills use a different mechanism and shouldn't carry a "Use when ..." phrase. No current skill tripped this, but the rule is now correct for the documented spec.
...
- `ia-brainstorming` Phase 2.5 restructures end-to-end. The three-bucket structure (Stated / Inferred / Out of scope) is now internal thinking only, never pasted into chat. The user sees a conversational scoping synthesis with four named sections, each render-conditional on having something to say: What we're building (always present, 1-3 sentences), Key trade-offs, What's not in scope, Call-outs. Routing depends on two signals: did any blocking question fire, and what tier did Phase 0 classify. Path A (Lightweight tier + no blocking) emits prose only and proceeds to doc-write in the same turn; Path B (Standard/Deep tier OR any blocking question) runs the full synthesis with confirmation gate. Tier-based bullet budget caps each section group. Per-section keep tests filter mechanical or inevitable items into the internal draft only.
- `ia-brainstorming` Phase 1 gains rigor probes (evidence / specificity / counterfactual / attachment as open-ended forced production, not menus) and an integration check before exit (combine stated answers and agent defaults; surface non-obvious downstream consequences as one probe each). Both patterns moved to `references/deep-interview.md` alongside the existing assumption-probing and second-order-effect rules; SKILL.md carries a one-line pointer that names them explicitly.
- `ia-brainstorming` body trimmed from ~5450 to ~4720 tokens (`-13%`) across the rest of the file: collapsed Phase 0's mirror-image signal lists into one decision rule, halved the info-dump example, consolidated Question Techniques, dropped two rationale-only sentences from Phase 2.5, tightened the Call-outs keep test, compressed Phase 3b's Spec Self-Review checklist, and trimmed the ideation lenses from 7 to 5. Semantic content preserved; the body is still over the 4K soft budget, accepted as load-bearing.
...
- `distillery/scripts/distiller.py` `_CANONICAL_TRIGGER_PATTERN` fix from the post-sync audit: the first draft put `for` in the trailing keyword group reachable via the `used?` past-tense form, which let `Tool used for X` and `Used for prompt refinement` pass the gate. Split the alternation so `for` is reachable only via imperative `use` (or compound `use this skill for`); past-tense `used?` is now restricted to the `when/after/before/whenever` branch, which doesn't appear in passive constructions naturally. Verified against 12 cases (6 imperative/compound true positives, 6 passive/noun-form true negatives).
- `ia-postgresql` JSONB Patterns Common-mistakes bullet for `jsonb_set + NULL`: the wiki addition claimed `jsonb_set(col, '{a,b}', NULL)` "sets the value to JSON null." Verified incorrect against PG semantics: `jsonb_set` is marked `STRICT`, so a bare SQL `NULL` makes the entire expression return SQL `NULL` and clobbers the column on `UPDATE`. To set JSON null the call must be `jsonb_set(col, '{a,b}', 'null'::jsonb)`. The bullet now distinguishes three cases (JSON null via explicit cast, SQL `NULL` clobber, `#-` as the actual delete answer).
- `ia-writing-tests` ephemeral-container example replaced `docker compose --rm` (not a real top-level flag, copy-paste hazard) with `docker compose run --rm <service>` for one-shot runs. The `down -v` form was already correctly documented in the same bullet list.
- `ia-writing` README Rules section no longer uses an em dash in the same paragraph that mandates `grep -c "—" README.md == 0`. The "Hybrid merge on rewrites" bullet now uses semicolons and a sentence split rather than an inline em dash. Modeling matters.
- `ia-brainstorming` Phase 2.5 skip rule re-anchored to Phase 0.2's existing "requirements already clear" detection (observable condition) instead of the undefined "no substance" criterion that briefly collided with Path A's announce-mode definition.
- `ia-brainstorming` Phase 2.5 Call-outs keep test gains an observable Phase-1-boundary check: if a call-out depends only on Phase 1 facts (no Phase 2 approach choice referenced, no agent default surfaced after dialogue), Phase 1's integration check failed and the call-out belongs upstream, not here. Replaces the judgement-only "should have asked in Phase 1" vs "post-dialogue consequence" distinction with a falsifiable test.
...
Sync from upstream CE plus reactive audit. Seven patterns adopted from EveryInc's plugin and harness session anomalies; twelve quality fixes follow-on. Brainstorm and plan flows gain a Stated/Inferred/Out-of-scope checkpoint before doc-write so scope misreads surface while still cheap. Swarm orchestration learns to queue overflow when the harness caps active subagents instead of silently dropping work. Compound-refresh checks inbound citations before archive so substantive references don't dangle. The negative-signal classifier sheds a narration false-positive that was diluting the corpus.
